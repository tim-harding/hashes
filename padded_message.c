...
PaddedMessage PaddedMessage_from_cstr(const char* message) {
    ...
    for (u32 i = message_length + 1,
             end = block_count * BLOCK_BYTES - MESSAGE_LENGTH_BYTES;
         i < end; i++) {
        buffer[i] = 0x00;
    }

    u64 message_length_in_bits = ((u64)message_length) * BITS_PER_BYTE;
    Block* last_block = &blocks[block_count - 1];
    last_block->long_word[BLOCK_U64S - 1] = message_length_in_bits;
...
void PaddedMessage_free(PaddedMessage* message) { free(message->blocks); }

void PaddedMessage_print_hex(PaddedMessage* message) {
    for (u32 block_i = 0; block_i < message->block_count; block_i++) {
        print_bytes((u8*)&message->blocks[block_i].byte, BLOCK_BYTES);
    }
}